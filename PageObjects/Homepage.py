...
class Homepage:
    ...
    def punchin_preactive_number(self, msisdn):
        ...
        assert "First top up required" in first_topup_text, "Failed to load first topup offers as Punch-In failed"
    ...
    def past_transaction(self, msisdn):
        ...
        self.exp_wait.until(expected_conditions.presence_of_element_located(self.past_transaction_search)).send_keys(
            msisdn)
        self.driver.find_element(By.CSS_SELECTOR, ".absolute.right-2.top-4").click()
        purchased_offers = self.exp_wait.until(expected_conditions.visibility_of_all_elements_located(
            (By.XPATH, '//div[@class="retailer-transaction-table mt-10"]')))
    ...
    def change_language(self):
        self.exp_wait.until(expected_conditions.presence_of_element_located((By.XPATH, "//span[text()='Settings']"))).click()
        dropdown=Select(self.driver.find_element(By.CSS_SELECTOR, ".drpDwnStyle"))
        dropdown.select_by_value("chg")
        language_text=self.exp_wait.until(expected_conditions.presence_of_element_located((By.XPATH, '//span[@class="text-xll font-bold"]'))).text
        assert language_text == "语言", "Language was not changed"
